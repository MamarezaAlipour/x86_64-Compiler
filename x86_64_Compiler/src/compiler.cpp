...
namespace x86_64 {
	...
	class Compiler::Impl final {
	  ...
	  private: // methods
		...
		void genREXPrefix(int8_t const& reg, Size size, int8_t const& index, int8_t const& base);
	  ...
	  public:
		template <class T>
		typename std::enable_if<std::is_signed<T>::value, bool>::type isByte(T value) {
		...
		template <class T>
		typename std::enable_if<std::is_signed<T>::value, bool>::type isDword(T value) {
	...
	Compiler::SymRef operator+(int64_t offset, const Compiler::SymRef& ref) { return ref + offset; }
	...
	Compiler::Ref::Ref(Ref const& ref) { *this = ref; }

	Compiler::Ref::Ref(Ref&& ref) { *this = std::move(ref); }
	...
	Compiler::Ref operator+(int64_t offset, Compiler::Ref const& ref) { return ref + offset; }
	...
	void Compiler::reset() { return m_impl->reset(); }

	void Compiler::rdata(std::string const& name, uint8_t const* data, std::size_t size) {
		return m_impl->rdata(name, data, size);
	}

	void Compiler::data(const std::string& name, const uint8_t* data, std::size_t size) {
	...
	Compiler::RegRef Compiler::reg(RegRef const& reg) const { return m_impl->reg(reg); }

	Compiler::MemRef Compiler::mem(int64_t disp) const { return m_impl->mem(disp); }
	...
	Compiler::SymRef Compiler::abs(std::string const& name) { return m_impl->abs(name); }
	...
	void Compiler::constant(uint16_t value) { return m_impl->constant(value); }

	void Compiler::constant(uint32_t value) { return m_impl->constant(value); }

	void Compiler::constant(uint64_t value) { return m_impl->constant(value); }

	void Compiler::constant(double value) { return m_impl->constant(value); }

	void Compiler::add(Ref const& src, Ref const& dst) { return m_impl->add(src, dst); }

	void Compiler::addb(uint8_t imm, Ref const& dst) { return m_impl->addb(imm, dst); }

	void Compiler::addw(uint16_t imm, Ref const& dst) { return m_impl->addw(imm, dst); }

	void Compiler::addl(uint32_t imm, Ref const& dst) { return m_impl->addl(imm, dst); }

	void Compiler::addq(uint64_t imm, Ref const& dst) { return m_impl->addq(imm, dst); }

	void Compiler::call(int32_t disp) { return m_impl->call(disp); }

	void Compiler::callw(int16_t disp) { return m_impl->callw(disp); }

	void Compiler::callq(int32_t disp) { return m_impl->callq(disp); }

	void Compiler::call(Ref const& ref) { return m_impl->call(ref); }

	void Compiler::callw(Ref const& ref) { return m_impl->callw(ref); }

	void Compiler::callq(Ref const& ref) { return m_impl->callq(ref); }

	void Compiler::lcall(Ref const& ref) { return m_impl->lcall(ref); }

	void Compiler::lcallw(Ref const& ref) { return m_impl->lcallw(ref); }

	void Compiler::lcalll(Ref const& ref) { return m_impl->lcalll(ref); }

	void Compiler::call(SymRef const& ref) { return m_impl->call(ref); }

	void Compiler::lcall(SymRef const& ref) { return m_impl->lcall(ref); }

	void Compiler::enter(uint16_t imm16, uint8_t imm8) { return m_impl->enter(imm16, imm8); }

	void Compiler::enterw(uint16_t imm16, uint8_t imm8) { return m_impl->enterw(imm16, imm8); }
	...
	void Compiler::leavew() { return m_impl->leavew(); }

	void Compiler::leaveq() { return m_impl->leaveq(); }

	void Compiler::mov(Ref const& src, Ref const& dst) { return m_impl->mov(src, dst); }

	void Compiler::mov(SymRef const& src, RegRef const& dst) { return m_impl->mov(src, dst); }

	void Compiler::mov(RegRef const& src, SymRef const& dst) { return m_impl->mov(src, dst); }

	void Compiler::movb(uint8_t imm, Ref const& dst) { return m_impl->movb(imm, dst); }

	void Compiler::movw(uint16_t imm, Ref const& dst) { return m_impl->movw(imm, dst); }

	void Compiler::movl(uint32_t imm, Ref const& dst) { return m_impl->movl(imm, dst); }

	void Compiler::movl(SymRef const& imm, Ref const& dst) { return m_impl->movl(imm, dst); }

	void Compiler::movq(uint64_t imm, Ref const& dst) { return m_impl->movq(imm, dst); }

	void Compiler::nop() { return m_impl->nop(); }

	void Compiler::pop(RegRef const& ref) { return m_impl->pop(ref); }

	void Compiler::popw(MemRef const& ref) { return m_impl->popw(ref); }

	void Compiler::popq(MemRef const& ref) { return m_impl->popq(ref); }

	void Compiler::push(uint32_t imm) { return m_impl->push(imm); }

	void Compiler::pushw(uint16_t imm) { return m_impl->pushw(imm); }

	void Compiler::pushq(uint32_t imm) { return m_impl->pushq(imm); }

	void Compiler::push(RegRef const& ref) { return m_impl->push(ref); }

	void Compiler::pushw(MemRef const& ref) { return m_impl->pushw(ref); }

	void Compiler::pushq(MemRef const& ref) { return m_impl->pushq(ref); }

	void Compiler::pushw(SymRef const& ref) { return m_impl->pushw(ref); }

	void Compiler::pushq(SymRef const& ref) { return m_impl->pushq(ref); }

	void Compiler::ret(uint16_t imm) { return m_impl->ret(imm); }

	void Compiler::ret() { return m_impl->ret(); }

	void Compiler::lret(uint16_t imm) { return m_impl->lret(imm); }

	void Compiler::lret() { return m_impl->lret(); }

	void Compiler::sub(Ref const& src, Ref const& dst) { return m_impl->sub(src, dst); }

	void Compiler::subb(uint8_t imm, Ref const& dst) { return m_impl->subb(imm, dst); }

	void Compiler::subw(uint16_t imm, Ref const& dst) { return m_impl->subw(imm, dst); }

	void Compiler::subl(uint32_t imm, Ref const& dst) { return m_impl->subl(imm, dst); }

	void Compiler::subq(uint64_t imm, Ref const& dst) { return m_impl->subq(imm, dst); }
	...
	void Compiler::Impl::rdata(std::string const& name, uint8_t const* data, std::size_t size) {
		std::size_t offset = section(RDATA).size();
		section(RDATA).push(data, size);
		pushSymbol(name, ".rdata", offset);
	}

	void Compiler::Impl::data(std::string const& name, uint8_t const* data, std::size_t size) {
	...
	const ByteArray& Compiler::Impl::getCode() const { return section(TEXT); }
		...
		for (auto& reloc : m_relocs)
			if (reloc.name == name) {
				if (reloc.type == SymRef::Type::Abs)
					*reinterpret_cast<uint32_t*>(section(TEXT).data() + reloc.offset) += value;
				else
					*reinterpret_cast<uint32_t*>(section(TEXT).data() + reloc.offset) +=
						value - reinterpret_cast<int64_t>(section(TEXT).data() + reloc.offset + 4);
			}
	}

	void Compiler::Impl::constant(uint8_t value) { gen(value); }

	void Compiler::Impl::constant(uint16_t value) { gen(value); }

	void Compiler::Impl::constant(uint32_t value) { gen(value); }

	void Compiler::Impl::constant(uint64_t value) { gen(value); }

	void Compiler::Impl::constant(double value) { gen(value); }

	void Compiler::Impl::add(Ref const& src, Ref const& dst) { instr(0x00, src, dst); }

	void Compiler::Impl::addb(uint8_t imm, Ref const& dst) { add(imm, dst); }

	void Compiler::Impl::addw(uint16_t imm, Ref const& dst) { add(imm, dst); }

	void Compiler::Impl::addl(uint32_t imm, Ref const& dst) { add(imm, dst); }

	void Compiler::Impl::addq(uint64_t imm, Ref const& dst) { add(imm, dst); }
	...
	void Compiler::Impl::call(int32_t disp) { callq(disp); }
	...
	void Compiler::Impl::enter(uint16_t imm16, uint8_t imm8) { enterq(imm16, imm8); }
	...
	void Compiler::Impl::leave() { leaveq(); }
	...
	void Compiler::Impl::mov(const Imm& imm, const Ref& dst) {
		if (dst.type == Ref::Type::Mem || (dst.reg.size == Size::Qword && isDword(imm.qword))) {
			...
			genb((dst.reg.size == Size::Byte ? 0xb0 : 0xb8) + (dst.reg.reg & c_x86_mask));
			gen(imm);
		}
	}
	...
	void Compiler::Impl::pushw(uint16_t imm) { push(imm); }
	...
	void Compiler::Impl::ret() { genb(0xc3); }
	...
	void Compiler::Impl::instr_no_w(uint8_t opcode, int8_t reg, Size size, const Ref& rm_ref) {
		...
		int8_t base = rm_ref.type == Ref::Type::Mem ? rm_ref.mem.base.reg : rm_ref.reg.reg;
	...
	void Compiler::Impl::genRef(int8_t reg, const MemRef& mem_ref) {
		...
		if (mem_ref.disp_specified || (mem_ref.base.reg & c_x86_mask) == 5) {
			if (isByte(mem_ref.disp) && mem_ref.base != NOREG && mem_ref.base != RIP) {
	...
	void Compiler::Impl::genb(uint8_t value) { gen(value); }

	void Compiler::Impl::genw(uint16_t value) { gen(value); }

	void Compiler::Impl::genl(uint32_t value) { gen(value); }

	void Compiler::Impl::genq(uint64_t value) { gen(value); }
